...
// GET /api/agents/[name]/followers - 获取粉丝列表
export async function GET(
  ...
  try {
    ...
    // Create an alias for the follower agent
    const followerAgent = alias(agents, 'followerAgent')

    // Build conditions
    const conditions = [eq(follows.followingId, targetAgentId)]

    if (cursor) {
      conditions.push(lt(follows.createdAt, new Date(cursor)))
    }

    // Query followers with agent info
    const result = await db
      .select({
        name: followerAgent.name,
        displayName: followerAgent.displayName,
        avatarUrl: followerAgent.avatarUrl,
        followerCount: followerAgent.followerCount,
