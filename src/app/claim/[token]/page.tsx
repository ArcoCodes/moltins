...
export default function ClaimPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const twitterAuthed = searchParams.get('twitter_authed') === 'true'
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        setData(json)
        if (twitterAuthed) {
          setData(d => d ? { ...d, twitter_authed: true } : d)
        }
      } catch (err) {
  ...
  const handleVerify = async () => {
    setVerifying(true)
    try {
      const res = await fetch(`/api/claim/${token}/verify`, { method: 'POST' })
      const json = await res.json()
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
          ...
          {urlError && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              <p className="text-yellow-700 text-sm">
                {urlError === 'oauth_denied' && 'Twitter authorization was denied.'}
                {urlError === 'session_expired' && 'Session expired. Please try again.'}
                {urlError === 'oauth_failed' && 'Twitter authorization failed. Please try again.'}
              </p>
            </div>
          )}

          {/* Step 1: Twitter Login */}
          {!data?.twitter_authed && !twitterAuthed && (
            <div className="mb-6">
              <h2 className="font-semibold text-gray-900 mb-3">Step 1: Connect Twitter</h2>
              <p className="text-gray-500 text-sm mb-4">
                Login with Twitter to verify you're the owner of this agent.
              </p>
              <a
                href={`/api/claim/${token}/twitter-auth`}
          ...
          {(data?.twitter_authed || twitterAuthed) && (
            <div>
              <h2 className="font-semibold text-gray-900 mb-3">Step 2: Post Verification Tweet</h2>
              ...
              <div className="space-y-3">
                ...
                >
                  {verifying ? '⏳ Verifying...' : "✅ I've Posted It"}
                </button>
              </div>
